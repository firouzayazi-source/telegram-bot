...
DEFAULT_SETTINGS={"show_workhours_in_sections":True,"show_datetime_footer":True,
    "show_workhours_menu":True,"show_catalog_menu":True,"show_ticket_menu":True,
...
def main_menu():
    ...
    if get_setting("show_ticket_menu"):    extra.append("💬 گفتگو با پشتیبانی")
...
def settings_kb():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"{t('show_ticket_menu')} گزینه پشتیبانی در منو",callback_data="stg_show_ticket_menu")],
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="back_to_admin": await query.message.edit_text("👑 پنل مدیریت",reply_markup=admin_menu())
        ...
        op="🟢 باز" if is_open_now() else "🔴 بسته"; wh=today_workhours_text() or ""
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for k,v in MENU_ITEMS.items():
        if text==v:
            ...
            full=build_message(v,content,k); kb=user_section_kb(k)
