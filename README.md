...
|`WEB_PASSWORD` |✅      |رمز ورود به پنل وب — **بدون آن پنل بالا نمی‌آید**     |
|`PANEL_USER`   |اختیاری|نام کاربری پنل — پیش‌فرض: `admin`                    |
|`PANEL_SECRET` |اختیاری|کلید امضای کوکی — اگر ست نشود هر ری‌استارت لاگ‌اوت می‌شوید|
|`WEB_PORT`     |اختیاری|پورت پنل وب — پیش‌فرض: `8080`. روی وی‌پی‌اس مشترک حتماً ست شود|
|`WEB_HOST`     |اختیاری|اینترفیس bind — پیش‌فرض: `0.0.0.0`                    |

> ⚠️ **امنیت:** `WEB_PASSWORD` هیچ مقدار پیش‌فرضی ندارد و اگر خالی باشد برنامه با خطای واضح متوقف می‌شود — پنل هرگز با رمز پیش‌فرض روی اینترنت باز نمی‌ماند. (متغیر قدیمی `PANEL_PASS` فقط برای سازگاری هنوز خوانده می‌شود.)
...
- `GET /api/stats` — بازدید بخش‌ها (از `stats.json`)
- `GET /api/sections`, `PUT /api/section/<key>/*` — مدیریت متن/دکمه‌های بخش‌ها
- `POST/DELETE /api/section/<key>/banner` — آپلود و حذف بنر (به تلگرام هم آپلود می‌شود)
